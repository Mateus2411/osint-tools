...
**Erro**: lxml não conseguia compilar no Windows devido à falta do Microsoft Visual C++ 14.0  
**Causa**: O requirements.txt limitava lxml a versões <5, forçando compilação de versões antigas

### 2. Solução Aplicada
**Arquivo modificado**: `requirements.txt` (linha 8)
```diff
- lxml>=4.9.2,<5
+ lxml>=4.9.2
```
...
**Por que funcionou**:
- Removeu a restrição de versão máxima
- Permitiu instalar lxml 6.0.2 (versão pré-compilada disponível)
- Evitou necessidade de compilação local

### 3. Instalação Corrigida para Windows
```bash
# Navegar para o diretório
cd spiderfoot

# Instalar dependências (com correção aplicada)
...
# Acessar interface
# Abrir navegador em: http://127.0.0.1:5001/
```

---

## 🚀 Como Usar o SpiderFoot
...
### Interface Web - Funcionalidades Principais

#### 1. New Scan (Novo Scan)
- **Target Types**: IP, Domain, Email, Phone, Bitcoin Address, etc.
- **Scan Types**:
...
- **Modules**: Selecionar específicos ou "All"
...
### Dashboard Principal
- **Browse**: Navegar por tipos de dados
- **Graph**: Visualização em grafo das conexões
- **Export**: CSV, JSON, GEXF para análise externa

### Tipos de Eventos Importantes
- **AFFILIATE_DOMAIN**: Domínios relacionados
- **IP_ADDRESS**: IPs descobertos
- **EMAILADDR**: Emails encontrados
- **VULNERABILITY**: Vulnerabilidades identificadas
- **MALICIOUS_IPADDR**: IPs maliciosos
- **SOCIAL_MEDIA**: Perfis sociais
...
- Configurar API keys para serviços premium
- Ajustar timeouts e threads

### 2. Correlação de Dados
- Engine YAML para correlacionar eventos
- 37 regras pré-definidas em `/correlations/`
- Detecta padrões suspeitos automaticamente

### 3. Exportação e Análise
```bash
# Via CLI para automação
py -3.14 sf.py -s exemplo.com -o json > resultado.json
py -3.14 sf.py -s exemplo.com -o csv > resultado.csv
```
...
### 1. Autenticação
- Por padrão SEM senha (desenvolvimento)
- Para produção: configurar autenticação
- Documentação: https://www.spiderfoot.net/documentation/#security

### 2. Rate Limiting
- Respeitar limites de APIs
- Configurar delays entre requests
- Usar proxies se necessário

### 3. Legal e Ético
- Usar apenas em domínios próprios ou autorizados
- Respeitar termos de serviço das APIs
...
- **Linha**: 8 
- **Mudança**: Removeu restrição <5 do lxml 
- **Motivo**: Compatibilidade com Windows sem compilador C++ 
- **Resultado**: Instalação bem-sucedida usando versão pré-compilada

**Agora você tem um SpiderFoot totalmente funcional para OSINT!** 🕵️‍♂️
